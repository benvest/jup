...
# split into 5x5 grid of 4x4 matrices then check each matric accross each row, down each column, and accross
# the diagonal(s?). Then just find the biggest of the group.

# a more simple approach would be to scan the rows, then scan the columns then scan all the diagonals by moving
# the index correctly zigging and zagging on down the diagonals that have atleast 4 elements.
